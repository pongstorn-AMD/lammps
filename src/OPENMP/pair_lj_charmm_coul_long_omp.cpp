...
#pragma omp parallel LMP_DEFAULT_NONE LMP_SHARED(eflag,vflag)
#endif
  {
    ...
    if (evflag) {
      if (eflag) {
        if (force->newton_pair) eval_PM<1,1,1>(ifrom, ito, thr);
        else eval<1,1,0>(ifrom, ito, thr);
      } else {
        if (force->newton_pair) eval_PM<1,0,1>(ifrom, ito, thr);
        else eval<1,0,0>(ifrom, ito, thr);
      }
    } else {
      if (force->newton_pair) eval_PM<0,0,1>(ifrom, ito, thr);
...
void PairLJCharmmCoulLongOMP::eval_PM(int iifrom, int iito, ThrData * const thr)
{
